...
def compute_rewards(df: pd.DataFrame, rule: object) -> pd.DataFrame:
    ...
    # 瓜分赛道配置（如有 tracks 则使用赛道配置，否则回退旧结构）
    tracks_cfg = cast(list[dict[str, object]], base_params.get("tracks", [])) if base_mode == "瓜分" else []
    if base_mode == "瓜分" and not tracks_cfg:
        pool_cfg_fallback = cast(dict[str, object], base_params.get("pool", {}))
        pool_total_fb = float(pool_cfg_fallback.get("total", 0))
        min_play_fb = float(pool_cfg_fallback.get("min_play", 0))
        tracks_cfg = [
            {
                "name": "默认瓜分赛道",
                "platforms": ["抖音/视频号", "小红书", "B站"],
                "merge_mode": "sum",
                "pool_tiers": [{"min_play": min_play_fb or 10000.0, "pool": pool_total_fb or 10000.0}],
                "coefficients": {},
                "floor": 0.0,
            }
        ]
    ...
    if base_mode == "瓜分":
        ...
        for t in tracks_cfg:
            platforms = [p for p in cast(list[str], t.get("platforms", [])) if p]
            if not platforms:
                platforms = platform_options
            tiers_raw = t.get("pool_tiers") or []
            tiers: list[dict[str, float]] = []
            for item in tiers_raw:
                try:
                    tiers.append({"min_play": float(item.get("min_play", 0)), "pool": float(item.get("pool", 0))})
                except Exception:  # noqa: BLE001
                    continue
            if not tiers:
                tiers = [{"min_play": 10000.0, "pool": 10000.0}]
            coeffs = {k: float(v) for k, v in cast(dict[str, object], t.get("coefficients", {})).items() if k}
            merge_mode = str(t.get("merge_mode", "sum"))
            floor_amt = float(t.get("floor", base_params.get("floor", 0) or 0))
            prepared_tracks.append(
                {
                    "name": str(t.get("name", "赛道")),
                    "platforms": platforms,
                    "pool_tiers": tiers,
                    "coefficients": coeffs,
        ...
        # 计算每条作品在各赛道的贡献播放量
        track_totals = [0.0 for _ in prepared_tracks]
        row_contribs: list[list[float]] = [[0.0 for _ in prepared_tracks] for _ in range(len(work))]

        for idx, row in work.reset_index(drop=True).iterrows():
            chan = str(row.get("渠道", ""))
            plays = float(row.get("播放量", 0))
            for t_idx, t in enumerate(prepared_tracks):
                if chan not in t["platforms"]:
                    continue
                coeff = t["coefficients"].get(chan, 1.0) if isinstance(t["coefficients"], dict) else 1.0
                contrib = plays * float(coeff)
                row_contribs[idx][t_idx] = contrib
                track_totals[t_idx] += contrib

        def select_pool_amount(tiers: list[dict[str, float]], total_play: float) -> tuple[float, float]:
            tiers_sorted = sorted(tiers, key=lambda x: x.get("min_play", 0))
            if not tiers_sorted:
                return 0.0, max(total_play, 1.0)
            qualified = [tier for tier in tiers_sorted if total_play >= tier.get("min_play", 0)]
            if qualified:
                chosen = qualified[-1]
                return float(chosen.get("pool", 0)), max(total_play, 1.0)
            # 未达最低档，按最低档奖金按比例分配
            first = tiers_sorted[0]
            return float(first.get("pool", 0)), max(first.get("min_play", 1.0), total_play, 1.0)

        pool_info = [select_pool_amount(t["pool_tiers"], track_totals[i]) for i, t in enumerate(prepared_tracks)]

        base_rewards = []
        for idx in range(len(work)):
            row_total = 0.0
            for t_idx, t in enumerate(prepared_tracks):
                contrib = row_contribs[idx][t_idx]
                if contrib <= 0:
                    continue
                pool_amt, denom = pool_info[t_idx]
                reward = pool_amt * (contrib / denom if denom else 0)
                floor_amt = float(t.get("floor", 0) or 0)
                if floor_amt > 0:
                    reward = max(reward, floor_amt)
                row_total += reward
            base_rewards.append(row_total)

        work["基础奖励"] = base_rewards
